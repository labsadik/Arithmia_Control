-- ============================================================================
-- OmniAgent Control — Database Schema
-- Mirrors the live Supabase database. Updated with every schema change.
--
-- Change log:
--   2026-08-22  Initial schema: agents, approvals, activity_events + demo data
-- ============================================================================
...
CREATE TABLE public.agents (
  ...
  status TEXT NOT NULL DEFAULT 'running',        -- running | idle | paused | error
...
-- ----------------------------------------------------------------------------
-- approvals — human-in-the-loop verification queue
-- ----------------------------------------------------------------------------
CREATE TABLE public.approvals (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  agent_id UUID REFERENCES public.agents(id) ON DELETE SET NULL,
  agent_name TEXT NOT NULL,
  action_type TEXT NOT NULL,                     -- stripe_payout | data_deletion | ...
  title TEXT NOT NULL,
  description TEXT,
  risk_level TEXT NOT NULL DEFAULT 'low',        -- low | high | critical
  status TEXT NOT NULL DEFAULT 'pending',        -- pending | approved | rejected
  payload JSONB NOT NULL DEFAULT '{}'::jsonb,    -- full request log shown in the UI
...
CREATE POLICY "Anyone can view approvals" ON public.approvals
  FOR SELECT TO anon, authenticated USING (true);
-- Only pending requests can be resolved, and only to approved/rejected.
CREATE POLICY "Anyone can resolve pending approvals" ON public.approvals
  FOR UPDATE TO anon, authenticated
  USING (status = 'pending') WITH CHECK (status IN ('approved', 'rejected'));

-- ----------------------------------------------------------------------------
-- activity_events — live agent activity stream
-- ----------------------------------------------------------------------------
CREATE TABLE public.activity_events (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  agent_name TEXT NOT NULL,
  status TEXT NOT NULL DEFAULT 'running',        -- running | completed | action_required | failed
...
-- ----------------------------------------------------------------------------
-- Demo data — 16 agents (14 running, cost sum $1,240, avg success 99.2%)
-- ----------------------------------------------------------------------------
...
-- Demo data — approval queue (3 pending, 1 approved, 1 rejected)
